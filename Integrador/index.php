<!DOCTYPE html>
<html lang="en">
<head>
<title>Escuela de Formación de Tecnólogos</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<link rel="icon" href="images/logo.ico">

<!-- css files -->
<link rel="stylesheet" href="css/bootstrap.css"> <!-- Bootstrap-Core-CSS -->
<link rel="stylesheet" href="css/style.css" type="text/css" media="all" /> <!-- Style-CSS --> 
<!-- //css files -->
</head>
    
    
<body>
<!-- header -->
        <form  id="formulario" action="<?php echo $_SERVER["PHP_SELF"]?>" method="post">
<div class="header">
	<div class="agile-top-header">
		<div class="logo">
            

			<h1 align="center"><a href="http://esfot.epn.edu.ec/"><span>Escuela</span></a></h1>
            <h1 align="center"><a href="http://esfot.epn.edu.ec/"><span>de</span></a></h1>
            <h1 align="center"><a href="http://esfot.epn.edu.ec/"><span>Formación de Tecnólogos</span></a></h1>
<!--
            <h1 align="center"><a href="http://esfot.epn.edu.ec/"><span></span></a></h1>
            <h1 align="center"><a href="http://esfot.epn.edu.ec/"><span></span></a></h1>
-->
           
		</div>
		<!-- navigation -->
		<div class="top-left">
<!--
			<div class="top-nav">
				<nav class="navbar navbar-default">
					<div class="logo">
						<a href="http://esfot.epn.edu.ec/"><img  class="img-responsive" src="images/LOGO_ESFOT_EPN.png" width="600" height="250"></a>
					</div>

				</nav>
			</div>
-->
		</div>
	</div>
    

	<div class="slider" align="center">
	    <!-- Slideshow 3 -->
	    <ul class="rslides" id="slider">
			<li>
			
				<img src="images/esfot1.JPG" alt="Dirección">
				
					<div class="slider-info">
						
						<div class="button">

							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
			<li >
				<img src="images/esfot2.JPG" alt="Oficina de profesores">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
<!--
			<li>
				<img src="images/esfot3.JPG" alt="Aulas">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
-->
            <li>
				<img src="images/esfot4.JPG" alt="Edificio 21">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
            <li>
				<img src="images/esfot5.JPG" alt="Oficina de profesores">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
<!--
            <li>
				<img src="images/esfot6.JPG" alt="Oficina de profesores">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
-->
            <li>
				<img src="images/esfot7.jpg" alt="Aulas">
					<div class="slider-info">

						<div class="button" >
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
            <li>
				<img src="images/esfot8.JPG" alt="Aulas">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
            <li>
				<img src="images/esfot9.JPG" alt="Aulas">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
            <li>
				<img src="images/esfot10.JPG" alt="Cancha de fútbol">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
<!--
            <li>
				<img src="images/esfot11.JPG" alt="Aulas">
					<div class="slider-info">

						<div class="button">
							<a href="#" data-toggle="modal" data-target="#myModal">Más<span class="glyphicon glyphicon-arrow-right" aria-hidden="true"></span></a>
						</div>
					</div>
			</li>
-->
	    </ul>
	    <!-- Slideshow 3 Pager -->
	    <ul id="slider3-pager">
	      <li><a href=""><img src="images/esfot1.1.jpg" alt=""></a></li>
	      <li><a href=""><img src="images/esfot2.1.jpg" alt=""></a></li>
<!--	      <li><a href=""><img src="images/esfot3.1.jpg" alt=""></a></li>-->
          <li><a href=""><img src="images/esfot4.1.jpg" alt=""></a></li>
          <li><a href=""><img src="images/esfot5.1.jpg" alt=""></a></li>
<!--          <li><a href=""><img src="images/esfot6.1.jpg" alt=""></a></li>-->
          <li><a href=""><img src="images/esfot7.1.jpg" alt=""></a></li>
          <li><a href=""><img src="images/esfot8.1.jpg" alt=""></a></li>
          <li><a href=""><img src="images/esfot9.1.jpg" alt=""></a></li>
          <li><a href=""><img src="images/esfot10.1.jpg" alt=""></a></li>
<!--          <li><a href=""><img src="images/esfot11.1.jpg" alt=""></a></li>-->
            
	    </ul>
    </div>	
          
</div>

	<!-- Modal -->
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog" >
		<div class="modal-dialog">
		<!-- Modal content-->
			<div class="modal-content">
				<div class="modal-header">
                                        
					<button type="button" onClick="Borrar ()" class="close" data-dismiss="modal" >&times;
                        
                     
                        
                        </button>
						<h2 align="center">Proyecto Integrador</h2>
						<br>
							<h3 align="center">Implementación de un sistema de video vigilancia mediante el 
								uso de cámaras IP en los pasillos  de las aulas de la Escuela de
								Formación de Tecnólogos (ESFOT) de la Escuela Politécnica Nacional
								</h3>	
							 
				<div class="row ">
                    
					
                    <div class="col-md-12">
                        <div class="price-div" >
                            <ul class="media-list">
                        <li class="media">
        <div class="media-body">
            
            <h3 class="media-heading" ><input  class="btn btn-primary btn-lg active" name="submit" id="fileInput" type="file" size="100" onchange="processFiles(this.files)"></h3>
          <p>
              <textarea name="campo" style="width:100%"    class="form-control custom-control"  id="fileOutput" type="file" cols ="100" rows="20" align="center" disabled  ></textarea>
          </p>
           
            </div>
                            </li>
    </ul>
                        </div>
                        
                    </div>
                </div>
				
				</div>
			</div>
						
		</div>
	</div>
	<!-- //Modal -->
<!-- //header -->

 <!-- Services -->
<div class="welcome agileits" id="services">
	<div class="container">
		<div class="welcome-main">
			<div class="welcome-top">
				<p>IMPLEMENTACIÓN DE UN SISTEMA DE VIDEO VIGILANCIA MEDIANTE 
                    EL USO DE CÁMARAS IP EN LOS PASILLOS  DE LAS AULAS DE LA 
                    ESCUELA DE FORMACIÓN DE TECNÓLOGOS (ESFOT) DE LA ESCUELA 
                    POLITÉCNICA NACIONAL</p>
			</div>
			
		</div>		
	</div>
</div>
            </form>
<!-- Services -->

<!--footer-->
<div class="w3l-footer">
    
	<div class="container">
        
		<div class="left-w3">
			
		</div>

		

		<div class="clearfix">
        
        
        </div>
        
		<div class="copyright-agile">
			<p>&copy; 2017 Fernando Moya, Karina S&aacute;nchez</p>
		</div>
	</div>
</div>
<!--//footer-->



<!-- js files -->
<!-- js -->
<script type="text/javascript" src="js/jquery-2.1.4.min.js"></script>
<script type="text/javascript" src="js/bootstrap.js"></script> <!-- Necessary-JavaScript-File-For-Bootstrap --> 
<!-- //js -->
	<!-- For-Banner -->
		 <script src="js/responsiveslides.min.js"></script>
		 <script>
			
		  </script>
	<!-- //For-Banner -->
	


    <script src="js/callTxt.js"></script>
		<script src="js/buttomTop.js"></script>
        <script src="js/openDocuments.js"></script>
		  <script src="js/banner.js"></script>
	

	<!-- start-smoth-scrolling -->
        
		<script type="text/javascript" src="js/move-top.js"></script>
		<script type="text/javascript" src="js/easing.js"></script>
		<script type="text/javascript" src="js/top.js"></script>
	<!-- start-smoth-scrolling -->
	
		<a href="#" id="toTop" style="display: block;"> <span id="toTopHover" style="opacity: 1;"> </span></a>
	<!-- //for-bottom-to-top smooth scrolling -->

<!-- //js files -->
</body>
</html>